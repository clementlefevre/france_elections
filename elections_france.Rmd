...
classSuffrages <- classIntervals(idf@data$X..Voix.Exp.1, 5, style = "quantile")
...
idf@data$suffrage_cat <- as.character(cut(idf@data$X..Voix.Exp.1, breaks = classSuffrages$brks, labels = palette, include.lowest = TRUE))

# On stocke l'information des classes pour créer une légende 
legende <- as.character(levels(cut(idf@data$X..Voix.Exp.1, breaks = classSuffrages$brks, include.lowest = TRUE, right = FALSE)))
...
```{r}
plot(idf, col = idf@data$suffrage_cat, border = "black")
