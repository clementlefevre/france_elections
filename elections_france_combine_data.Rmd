...
corr <- round(cor(df.all %>% select(one_of(c(candidats)),matches('NoJob|Q213'))%>% na.omit()),1)
...
# add logarithmic Ratio for Misc Features

Nb.Columns<- grep('Nb.',colnames(df.all),value = T)
...
```{r}
bretagne<-df.all %>% filter(REG==53) #%>% filter(!is.na(Q3_Q1))
str(bretagne)
dim(bretagne)
model1<-lm(LE.PEN~. ,data=bretagne )
summary(model1)
```
