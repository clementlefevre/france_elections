...
```{r}
ggplot(df.all.clean  %>% filter(NCC =="BRETAGNE") %>% filter(Population>10),aes(x=Q3_Q1,y=LE.PEN,col=NCC))+geom_jitter(alpha=.3) #+geom_smooth(method = 'lm')
ggplot(df.all.clean  %>% filter(Code.Insee.de.la.région %in% c(32,53,93)) %>% filter(Population<100000),aes(x=log(Population),y=(MACRON),col=NCC))+geom_jitter(alpha=.3) #+geom_smooth(method = 'lm')
```
