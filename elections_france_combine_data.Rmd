...
```
```{r}
df.all_numeric<-df.all %>% select(-contains('C11')) %>% select(-contains('P11')) %>% select_if(is.numeric)
str(df.all_numeric)
```

```{r}
candidats_one<-candidats[!candidats %in% c('LE.PEN')]
candidats_one
model1<-lm(LE.PEN~.,data=df.all_numeric %>% select(-one_of(candidats_one)))
```
