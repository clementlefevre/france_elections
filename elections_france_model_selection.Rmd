...
```{r}
cor.matrix<-cor(df_numeric %>% filter(!is.na(LE.PEN))%>% filter(Population<100) ) 
View(as.data.frame(cor.matrix) %>% select(LE.PEN))
```
```{r}
ggplot(df_numeric%>% filter(!is.na(LE.PEN)) %>% filter(ratio_immigrants>0)   ,aes(ratio_immigrants,LE.PEN))+ geom_point()
```
