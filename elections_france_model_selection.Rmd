...
plot_corr<-function(df,feature,ratio,pop_threshold,candidate,title){
 ...
 df<-  df[df[,feature]<1,]  %>%filter(Population>pop_threshold) %>% filter(REG %in% c(53,93,22,31))
  ...
  p<-ggplot(df,aes_string(x=x_data,y=candidate,col="NOM_REG",size="Population"))+ geom_jitter(alpha=.7)  +theme_fivethirtyeight() +
    theme(axis.title = element_text()) + scale_colour_economist()+ylab(paste0('1st Round - ',candidate,' (%)'))+ xlab('Worker rate in active population (%)')+theme(legend.direction='vertical',legend.position='top',legend.box='horizontal')+ggtitle(paste0('Cities with population>',pop_threshold))+labs(caption = "©Clément Lefèvre 2017 (Sources : Ministère de l'Intérieur, INSEE)")+scale_colour_manual(values = c("#f04723", "#f48d04", "#00a1ce"))+labs(size = "Population",color=' ')+ theme(legend.title=element_text(size=8))
  return(p)
}

p0<-plot_corr(df,"Job_OUVRIER_ratio",100,1000,'LE.PEN',"")
p1<-plot_corr(df,"Job_OUVRIER_ratio",100,1000,'MACRON','')
p0
p1
